...
file = "Input data/RBs_p600_050522.csv" # define input file
satellite_inclinations = np.genfromtxt(file)
casualty_expectation_file = "results_" + file # for output filenames
weighting_function_file = file +"_weighting_function"
...
GPW4 = np.genfromtxt("Input data/GPW4_2020.csv", delimiter=' ') # import and clean GPW4 data
GPW4 = np.where(GPW4==-9999, 0, GPW4)

print("Sanity check: Population of earth = ", np.sum(np.sum(GPW4)))
